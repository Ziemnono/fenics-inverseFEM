#!/usr/bin/env python
# description     :Perform forward and inverse simulations for a 3D sagging cube,
#                 in this programm, we run multiple simulations with fixed parameters 
#                 but different mesh refinements. 
...
def main():
    ...
    mesh_accuracy = np.arange(1, 20)
    total_mse = []
    nb_elements = []
    for i in mesh_accuracy:
        print(i)
    ...
    #     nb_elements.append(mesh.cells().shape[0])

    # with open('mesh_analysis.npy', 'wb') as f:
    #     np.save(f, np.array([np.array(nb_elements), np.array(total_mse)]))
