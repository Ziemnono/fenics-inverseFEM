...
#				  programm, we apply gravity on a soft beam
...
psi = Constant(C_01)*(J**(-2.0/3.0)*I_C - 3) + Constant(C_10)*(J**(-4.0/3.0)*II_C - 3) + p_*(J - 1) - (1.0/(4.0*Constant(D)))*p_**2

# second Piola-Kirchoff conjugate with incremental Green-Lagrange
S = 2.0*diff(psi, C)
D_E = derivative(E, u_p_, v)
G = inner(S, D_E)*dx + inner(((J - 1) - p_/Constant(2.0*D)), p_t)*dx - inner(Constant(g)*rho_e, -u_t[1])*dx
...
with XDMFFile("results/forward_deformed_configuration"+mesh_file+".xdmf") as xdmf:
    xdmf.write(mesh)
