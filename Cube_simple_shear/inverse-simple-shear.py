...
mesh = Mesh()
with XDMFFile("results/forward_configuration_simple_shear.xdmf") as infile:
    infile.read(mesh)
...
indices = np.loadtxt("results/simple_shear_bc_index.txt").astype(int).tolist()
for i in indices:
...
ALE.move(mesh, u_)
with XDMFFile("results/inverse_configuration_simple_shear.xdmf") as xdmf:
    xdmf.write(mesh)
