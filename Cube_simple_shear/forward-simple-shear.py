...
#				  programm, we reproduce the simple shear experiment
...
with XDMFFile('results/initial_configuration_simple_shear.xdmf') as xdmf:
    xdmf.write(mesh)
...
r = Expression(("k*x[1]", "0.0", "0.0"), k=k, degree=1)
...
np.savetxt("results/simple_shear_bc_index.txt", bc_index, delimiter=",")

solver_parameters = {"newton_solver": {"linear_solver": "mumps"}}

solve(G == 0, u_, J=J, bcs=bcs, solver_parameters=solver_parameters)
ALE.move(mesh, u_)
with XDMFFile("results/forward_configuration_simple_shear.xdmf") as xdmf:
    xdmf.write(mesh)
