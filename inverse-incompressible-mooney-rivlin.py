...
G = inner(sigma, sym(grad(u_t)))*dx + inner(((J - 1) - p_/Constant(2.0*D)), p_t)*dx - inner(Constant(g)*rho_e, -u_t[1])*dx
...
with XDMFFile("results/initial_configuration"+mesh_file+".xdmf") as xdmf:
    xdmf.write(mesh)
